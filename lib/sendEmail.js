...
const generateEmailContent = (data) => {
  ...
  return {
    ...
    <h2>${companyName}: A client Requesting from Get demo on RougeChatDB</h2> 
...
const sendMessage = async (data) => {
  try {
    const res = await transporter.sendMail({
      ...
      subject: "RougeChatDB: A client requesting from Get Demo on RougeChatDB",
    });
    ...
    if (res.accepted.length > 0) {
      return {
        status: true,
        message: "Thank you for your message. will be contact soon.",
        data: res,
      };
    }
